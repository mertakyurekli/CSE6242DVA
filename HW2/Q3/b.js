    ...
    d3.csv("boardgame_ratings.csv").then(function(data) {
        ...
        for (let i = 0; i < categories.length; i++) {
            ...
            if (subCategories.includes(categories[i])) {      // Add dots/circles to each line
                svg2.selectAll('myCircles')        // Add data label to circles
                    .data(data)
                    .enter()
                ...
                svg2.selectAll('myCircles')
                    ...
                    .attr("y", function (d, j) {
                        if (j % 3 === 2) {
                            return y(d[categories[i] + '=count']) + 3;
                        }
                    })
                    .attr('font-size', '8px')
                    .attr('stroke', 'white')
        ...
        var circleLegend = svg2.selectAll('.circleLegend')
            .data(legend_keys)
            .enter()
            .append('g')
        ...
        svg2.selectAll('.circleLegendIcon')        // Add data label to circles
            .data(legend_keys)
            .enter()
            .append('circle')               // A better way to concatenate the following '.attr's can be found here: https://stackoverflow.com/questions/20822466/how-to-set-multiple-attributes-with-one-value-function
            .attr('fill', 'black')
            ...
            .attr('cy', height - 20)
            .attr('r', 15)
        svg2.selectAll('.circleLegendIcon')
            .data(legend_keys)
            .enter()
            .insert('text')
            .text('rank')
