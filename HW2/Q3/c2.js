...
d3.csv("boardgame_ratings.csv").then(function(data) {
    ...
    y.domain([1e-6, d3.max(data, function (d) {
    ...
    // Add valueline1 to 'path' where 'valueline1' represents 'Catan'. So on and so forth:
        for (let i = 0; i < categories.length; i++) {
            ...
            if (subCategories.includes(categories[i])) {      // Add dots/circles to each line
                svg32.selectAll('myCircles')        // Add data label to circles
                    .data(data)
                    .enter()
        ...
        var circleLegend = svg32.selectAll('.circleLegend')
                                .data(legend_keys)
                                .enter()
                                .append('g')
        ...
        svg32.selectAll('.circleLegendIcon')        // Add data label to circles
             .data(legend_keys)
             .enter()
             .append('circle')               // A better way to concatenate the following '.attr's can be found here: https://stackoverflow.com/questions/20822466/how-to-set-multiple-attributes-with-one-value-function
             .attr('fill', 'black')
             ...
             .attr('cy', height-20)
             .attr('r', 15)
        svg32.selectAll('.circleLegendIcon')
            .data(legend_keys)
            .enter()
            .insert('text')
            .text('rank')
        ...
        var usernameLegend = svg32.selectAll('username')
            .data(['GT Username: yyu441'])
            .enter()
            .append('text')
        svg32.append('text')
