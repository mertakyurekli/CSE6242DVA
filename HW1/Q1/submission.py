...
class TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        # Method about how to use TMDb API: https://stackoverflow.com/questions/54164540/using-api-with-http-client-to-parse-imdb-movies-database
        domain = "api.themoviedb.org"
        suffix = '/3/movie/' + movie_id + '/credits?api_key=' + self.api_key
        ...
        if exclude_ids is not None:
            for exclude_id in exclude_ids:
                exclude_ids_str.append(str(exclude_id))
        # We should first limit the size of cast member
        if limit and len(data) >= limit:
            data = data[:limit]
            if exclude_ids_str is None:
                return data
            # Then, some cast members should be excluded:
            elif exclude_ids_str or len(exclude_ids_str) == 0:
                for d in data:
                    if str(d['id']) not in exclude_ids_str:
                        output.append(d)
                return output
        if not limit and exclude_ids_str:
            for d in data:
                if str(d['id']) not in exclude_ids_str:
                    output.append(d)
            return output
        if not limit and not exclude_ids_str:
            return data
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        # Method about how to use TMDb API: https://stackoverflow.com/questions/54164540/using-api-with-http-client-to-parse-imdb-movies-database
        domain = "api.themoviedb.org"
        suffix = '/3/person/' + person_id + '/movie_credits?api_key=' + self.api_key
        conn = http.client.HTTPSConnection(domain)
        conn.request("GET", suffix)
        response = conn.getresponse()
        output = []
        ...
        for d in castData:
            currDict = {}
            if (vote_avg_threshold and d['vote_average'] >= vote_avg_threshold) or (not vote_avg_threshold):
                currDict['id'] = d['id']
                currDict['title'] = d['title']
                currDict['vote_average'] = d['vote_average']
                currDict['credit_id'] = d['credit_id']
...
if __name__ == "__main__":
    ...
    print(len(tmdb_api_utils.get_movie_cast(movie_id='603', limit=3, exclude_ids=['2975'])),
          tmdb_api_utils.get_movie_cast(movie_id='603', limit=3, exclude_ids=['2975']))
    print(len(tmdb_api_utils.get_movie_credits_for_person(person_id='2975', vote_avg_threshold=8.0)),
          tmdb_api_utils.get_movie_credits_for_person(person_id='2975', vote_avg_threshold=8.0))
